...
assert(/@media \(min-width: 1501px\) and \(max-height: 760px\)/.test(audioCss), "Audio & Media must include an explicit short wide-display layout");
...
assert(/Move " \+ row\.title \+ " up/.test(product) && /Change " \+ row\.title \+ " size; current size/.test(product), "Layout Editor controls must name the panel they affect");
...
assert(/grid-template-areas:\s*"handle title actions"/.test(productCss) && !/product-layout-row__handle\s*\{\s*display:\s*none/.test(productCss), "Layout Editor rows must keep a useful title column and visible handle");
