...
public sealed class AppPathsTests
{
    ...
    public void DashboardUri_UsesItsConfiguredLocalOriginForTheBridge()
    {
        ...
        var dashboard = BridgeManager.BuildDashboardUri(baseUri, "20260818-02", advanced: false);
        var settings = BridgeManager.BuildDashboardUri(baseUri, "20260818-02", advanced: true);

        Assert.Multiple((Action)(() =>
        {
            Assert.That(dashboard.GetLeftPart(UriPartial.Path), Is.EqualTo("http://127.0.0.1:54321/dashboard.html"));
            Assert.That(dashboard.Query, Is.EqualTo("?bridge=http%3A%2F%2F127.0.0.1%3A54321&v=20260818-02"));
            Assert.That(settings.Query, Is.EqualTo("?advanced=1&bridge=http%3A%2F%2F127.0.0.1%3A54321&v=20260818-02"));
        }));
    }
