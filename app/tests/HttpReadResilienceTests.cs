...
public sealed class HttpReadResilienceTests
{
    ...
    [Test]
    public async Task SendAsync_RecomputesContentLengthAfterBuffering()
    {
        using var handler = new ContentLengthHandler("ok", 100);
        using var client = new HttpClient(handler);
    ...
    private sealed class ContentLengthHandler(string content, long reportedLength) : HttpMessageHandler
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
